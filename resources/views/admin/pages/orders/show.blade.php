                ...
                <div class="col-md-9">
                    <table class="table table-bordered table-striped table-hover m-0">
                        ...
                        <tr>
                            ...
                            <td class="text-bold">
                                {{ $order->province_name }} / {{ $order->regency_name }} / {{ $order->district_name }}
                    ...
                    <form action="{{ route('admin::orders.set-status', $order->order_id) }}" method="POST">
                        ...
                        <div class="form-group">
                            <select name="status" class="form-control">
                                ...
                                <option value="shipping" {{ $order->status == 'shipping' ? 'selected' : '' }}>Pengiriman
                                </option>
                                <option value="finish" {{ $order->status == 'finish' ? 'selected' : '' }}>Selesai
                                </option>
                                <option value="canceled" {{ $order->status == 'canceled' ? 'selected' : '' }}>Dibatalkan
                                </option>
                                <option value="rejected" {{ $order->status == 'rejected' ? 'selected' : '' }}>Ditolak
                                </option>
                            </select>
                        </div>
                            ...
                            <tr>
                                <td>
                                    <a href="{{ $payment_confirmation->url_payment_proof }}" target="_blank">
                                        <img class="img-thumbnail" src="{{ $payment_confirmation->url_payment_proof }}"
